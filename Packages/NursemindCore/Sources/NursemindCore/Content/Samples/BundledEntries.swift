...
extension ContentRegistry {
    ...
    private static func bundledChunk04() -> [LibraryEntry] {
        ...
        // MARK: Drips — batch 3+4 (inodilation, sedation, HIT anticoagulation, rhythm, OB, volume, osmotherapy, hemostasis)
        entries.append(.drip(MilrinoneDripSample.entry))
        entries.append(.drip(DexmedetomidineDripSample.entry))
        entries.append(.drip(MidazolamDripSample.entry))
        entries.append(.drip(ArgatrobanDripSample.entry))
        entries.append(.drip(BivalirudinDripSample.entry))
        entries.append(.drip(LidocaineDripSample.entry))
        entries.append(.drip(IsoproterenolDripSample.entry))
        entries.append(.drip(MagnesiumSulfateDripSample.entry))
        entries.append(.drip(FurosemideDripSample.entry))
        entries.append(.drip(ClevidipineDripSample.entry))
        entries.append(.drip(HypertonicSalineDripSample.entry))
        entries.append(.drip(TranexamicAcidDripSample.entry))
        // MARK: Labs
    ...
    private static func bundledChunk11() -> [LibraryEntry] {
        ...
        // MARK: Scenarios — v4 expansion (management of care + deterioration archetypes)
        entries.append(.scenario(PrioritizationScenarioSample.entry))
        entries.append(.scenario(DelegationScenarioSample.entry))
        entries.append(.scenario(AnaphylaxisScenarioSample.entry))
        entries.append(.scenario(TransfusionReactionScenarioSample.entry))
        entries.append(.scenario(OpioidOversedationScenarioSample.entry))
        entries.append(.scenario(HypoglycemiaScenarioSample.entry))
        entries.append(.scenario(CovertHemorrhageScenarioSample.entry))
        entries.append(.scenario(PulmonaryEmbolismScenarioSample.entry))
        entries.append(.scenario(SuicideRiskScenarioSample.entry))
        entries.append(.scenario(ChestTubeEmergencyScenarioSample.entry))
        // MARK: Scenarios — v5 expansion (specialty balance: peds, OB, onc, renal, neuro, ortho, airway, safety)
        entries.append(.scenario(PedsRespiratoryDistressScenarioSample.entry))
        entries.append(.scenario(NeutropenicFeverScenarioSample.entry))
        entries.append(.scenario(DialysisDayScenarioSample.entry))
        entries.append(.scenario(SeizureResponseScenarioSample.entry))
        entries.append(.scenario(AnticoagulatedHeadInjuryScenarioSample.entry))
        entries.append(.scenario(RestraintSafetyScenarioSample.entry))
        entries.append(.scenario(NeonatalFeverScenarioSample.entry))
        entries.append(.scenario(TracheostomyEmergencyScenarioSample.entry))
        entries.append(.scenario(CompartmentSyndromeScenarioSample.entry))
        entries.append(.scenario(MagnesiumToxicityScenarioSample.entry))
    ...
    private static func bundledChunk43() -> [LibraryEntry] {
        ...
        // MARK: Round 35 — Drugs (by-name workhorses: NSAIDs, antiemetics, steroid, nitrate, benzos, emergency epi, NPH, ticagrelor)
        entries.append(.drug(IbuprofenSample.entry))
        entries.append(.drug(MeloxicamSample.entry))
        entries.append(.drug(DexamethasoneSample.entry))
        entries.append(.drug(MetoclopramideSample.entry))
        entries.append(.drug(ProchlorperazineSample.entry))
        entries.append(.drug(IsosorbideMononitrateSample.entry))
        entries.append(.drug(DiazepamSample.entry))
        entries.append(.drug(AlprazolamSample.entry))
        entries.append(.drug(ClonazepamSample.entry))
        entries.append(.drug(EpinephrineSample.entry))
        entries.append(.drug(InsulinNPHSample.entry))
        entries.append(.drug(TicagrelorSample.entry))
        // MARK: Round 36 — Drugs (tier-2 by-name gaps: beta-blockers, verapamil, TCA, coxib, relaxants, antitussive, fosphenytoin)
        entries.append(.drug(PropranololSample.entry))
        entries.append(.drug(AtenololSample.entry))
        entries.append(.drug(VerapamilSample.entry))
        entries.append(.drug(AmitriptylineSample.entry))
        entries.append(.drug(CelecoxibSample.entry))
        entries.append(.drug(TizanidineSample.entry))
        entries.append(.drug(MeclizineSample.entry))
        entries.append(.drug(BenzonatateSample.entry))
        entries.append(.drug(FosphenytoinSample.entry))
