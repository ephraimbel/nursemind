...
for e in entries:
    ...
    v['nclexTags'] = dict(category=e['category'], subcategory=e['subcategory'], priorityConcept=e['concept'])
    if e['kind'] == 'diagnosis':
        for k in ['definition', 'pathophysiology']:
            v[k] = attributed(e[k])
        for k in ['presentation', 'diagnosticCriteria', 'priorityAssessments', 'commonInterventions', 'watchFor']:
            v[k] = [attributed(b) for b in e[k]]
    else:
        v['eyebrow'] = 'REFERENCE · COMMON CONDITIONS'
...
actual = extract('NM-COMMON-EVIDENCE:')
assert len(actual) == len(expected)
for a, e in zip(actual, expected):
    assert a == e, 'Compiled entry differs: ' + str(e)
audit = extract('NM-COMMON-AUDIT:')
